...
CountType getCol(const CountType &index, const CountType &width)
{
    return index % width;
}
template <typename CountType>
CountType getRow(const CountType &index, const CountType &width)
{
    return index / width;
}
    ...
    const uint8_t &&neighbor)
{
    ...
    std::vector<std::array<int8_t, 2>> neighborShit;
    ...
    {
    case 4:
        neighborShit = {{0, 1}, {1, 0}, {1, 2}, {2, 1}};
        break;
    case 8:
        neighborShit = {{0, 0}, {0, 1}, {0, 2}, {1, 0}, {1, 2}, {2, 0}, {2, 1}, {2, 2}};
        break;
    }

    // 可能需要返回的值用 py::array
    py::array_t<CountType> parMap = py::array_t<CountType>({height, width});
    CountType *parents = parMap.mutable_data();
    // 不需要返回的值用 vector
    std::vector<CountType> area;
    area.reserve(size);
    std::vector<CountType> length;
    length.reserve(size);
...

    {
        ...
        for (CountType i = 0; i < size; ++i)
        {
            ...
            area[p] = 1;
            length[p] = 0;
            for (auto &&[neSY, neSX] : neighborShit)
            {
                // 附近点是否超出图像范围
                // 由于数据类型为正整数，因此 +1 防止负数出现
                if (pY + neSY <= 0 or pX + neSX <= 0 or pY + neSY > height or pX + neSX > width) continue;
                CountType neP{(pY + neSY - 1) * width + (pX + neSX - 1)};
                CountType facePY{pY + (pY + neSY - 1)}, facePX{pX + (pX + neSX - 1)};                
                if (!visited[neP])
                {
                    if (facePY % 2 != facePX % 2)
                    {
                        length[p] += 1;
                    }
                    continue;
...
PYBIND11_MODULE(tree_trans, m) // 创建一个 Python 模块，名为 tree_trans ，用变量 m 表示
{
    m.doc() = "pybind11 example module"; // 给模块 m 添加说明文档
    m.def("min_max_tree",
          &MinMaxTree<uint8_t, uint32_t, py::array::c_style | py::array::forcecast>,
          py::arg("img"),
          py::arg("tree_type") = "mintree",
          py::arg("neighbor") = 8); // 给模块 m 定义一个函数，名为 sum ，绑定到 C++ 代码中的 sum 函数
}
