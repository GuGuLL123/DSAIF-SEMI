...
class Encoder_Without_Skip(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False):
        super(Encoder_Without_Skip, self).__init__()
        self.has_dropout = has_dropout
        convBlock = ConvBlock if not has_residual else ResidualConvBlock
...
class Decoder_Without_Skip(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, up_type=0):
        super(Decoder_Without_Skip, self).__init__()
        self.has_dropout = has_dropout
...
class VNet_feature(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, up_type=0):
        super(VNet_feature, self).__init__()

        self.encoder = Encoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder1 = Decoder_feature(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual,
                                        up_type)

    def forward(self, input):
        features = self.encoder(input)
        feature_out, out_seg1 = self.decoder1(features)
        return feature_out, out_seg1


class VNet_Without_Skip(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, up_type=0):
        super(VNet_Without_Skip, self).__init__()

        self.encoder = Encoder_Without_Skip(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        self.decoder1 = Decoder_Without_Skip(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual,
                                             up_type)
